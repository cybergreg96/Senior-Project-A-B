...
class Bunny implements Viewable {
	...
	public void BUNNYaim() {
		...
		double it = theta - Math.toRadians(angle) + random;
	...
	void handle(final long nanos) {
		...
		System.out.println(TimeUnit.NANOSECONDS.toSeconds(first) + " " + TimeUnit.NANOSECONDS.toSeconds(nanos) + " " + TimeUnit.NANOSECONDS.toSeconds(elapsed));
